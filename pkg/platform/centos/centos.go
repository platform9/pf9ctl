...
func (c *CentOS) Check() []platform.Check {
	...
	checks = append(checks, platform.Check{"Check if firewalld service is running", true, result, err, fmt.Sprintf("%s", err)})
...
func (c *CentOS) checkFirewalldIsRunning() (bool, error) {
	_, err := c.exec.RunWithStdout("bash", "-c", "systemctl status firewalld | grep 'active (running)'")
	if err != nil {
		return true, nil
	} else {
