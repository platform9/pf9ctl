...
func (c *CentOS) Check() []platform.Check {
	...
	result, err = c.checkIfSystemIsBootedWithSystemd()
...
func (c *CentOS) checkIfSystemIsBootedWithSystemd() (bool, error) {
	_, err := c.exec.RunWithStdout("bash", "-c", "pidof systemd | grep '1'")
	if err != nil {
