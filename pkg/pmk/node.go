...
func isSudoUser(os string, exec cmdexec.Executor) (bool, error) {
	var result string
	var err error
	if os == "debian" {
		result, err = exec.RunWithStdout("bash", "-c", "getent group sudo | cut -d: -f4 | tr -d '\\n'")
	} else if os == "redhat" {
		result, err = exec.RunWithStdout("bash", "-c", "getent group sudo | cut -d: -f4 | tr -d '\\n'")
	}
	...
	currentUser, err := exec.RunWithStdout("bash", "-c", "echo $USER")
	if err != nil {
		return false, err
	}
