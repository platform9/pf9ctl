...
var checkAmazonProviderCmd = &cobra.Command{
	Use:   "check-amazon-provider username access-key-id secret-access-key",
	...
	Args: func(checkGoogleProviderCmd *cobra.Command, args []string) error {
		if len(args) > 3 {
			return errors.New("Only username, keyID and secretKey are required")
		} else if len(args) < 3 {
...
func checkAmazonProviderRun(cmd *cobra.Command, args []string) {
	...
	if err := pmk.CheckAmazonPovider(awsUsername, awsKeyID, awsSecretKey); err != nil {
